...
void ReshapeGL (int width, int height)									// Reshape The Window When It's Moved Or Resized
{
	...
	gluPerspective (FRUSTUM_ANGLE, (GLfloat)(width)/(GLfloat)(height),	// Calculate The Aspect Ratio Of The Window
					FRUSTUM_ZNEAR, FRUSTUM_ZFAR);
...
BOOL CreateWindowGL (GL_Window* window)									// This Code Creates Our OpenGL Window
{
	...
	PIXELFORMATDESCRIPTOR pfd =											// pfd Tells Windows How We Want Things To Be
	{
		...
		(BYTE)window->init.bitsPerPixel,								// Select Our Color Depth
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (MessageBox (HWND_DESKTOP, "Fullscreen?", "", MB_YESNO | MB_ICONQUESTION) == IDYES) {
		window.init.isFullScreen = TRUE;								// If Not, Run In Windowed Mode
	}
#endif
	...
	while (g_isProgramLooping)											// Loop Until WM_QUIT Is Received
	{
		...
		if (CreateWindowGL (&window) == TRUE)							// Was Window Creation Successful?
		{
			...
			if (init(&window, &keys) == FALSE)					// Call User Intialization
			{
				// Failure
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						if (window.isVisible) {
							...
							if (update(tickCount - window.lastTickCount)) {
								window.lastTickCount = tickCount;
								drawScene();
							}
						} else WaitMessage();							// Window is minimised
					}
				}														// Loop While isMessagePumpActive == TRUE
			}															// If (Initialize (...

			// Application Is Finished
			dealloc();											// User Defined DeInitialization
